...
class Trainer:
    ...
    def Train(self):
        ...
        # print(self.device);
        print('Starting ACDNetv2_20 model training for SPLIT-{}'.format(self.split));
        acdnetv220_conf = [7, 20, 10, 14, 22, 31, 35, 41, 51, 67, 69, 48]; #ACDNetV2 has 48 filters in the last layer
    ...
    def __save_model(self, acc, epochIdx, net):
        if acc > self.bestAcc:
            dir = os.getcwd();
            fname = "{}/models/20_48_f{}_{:.2f}_{}.pt";
    ...
    def TestModel(self):
        net = models.GetACDNetModelV2(self.opt.inputLength, self.opt.nClasses, self.opt.fs).to(self.device);
        lossFunc = torch.nn.KLDivLoss(reduction='batchmean');
        dir = os.getcwd();
        net_path = '{}/models/run-1/sp{}_finch_model_*.pt'.format(dir, self.split,);
        print(net_path)
...
if __name__ == '__main__':
    opt = opts.parse();
    opt.datafile='btftpfp_wav{}.npz';
    # trainer = Trainer(opt, 1);
    # trainer.TestModel();
    # exit();
