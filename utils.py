...
def single_crop(input_len):
    def f(sound):
        org_len = len(sound)
        start = 0;
        # start = random.randint(0, org_len - input_len)
        # return sound[start: start + size]

        sound = [sound[start: start + input_len]];
        return np.array(sound);

    return f;

def conditional_pad(input_length):
    def f(sound):
        pad_length = (input_length - len(sound)) if len(sound) < input_length else 0;
        if pad_length > 0:
            sound = np.pad(sound, (0, pad_length), 'constant');
        # sound = np.pad(sound, input_length//2, 'constant');
        return sound;
