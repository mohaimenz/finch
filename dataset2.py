...
class Generator():
    ...
    def __init__(self, samples, labels, options, train=True):
        ...
        self.opt = options;
        self.batch_size = 64 if train == True else 64//self.opt.nCrops;
    ...
    def __getitem__(self, batchIndex):
        ...
        return torch.tensor(batchX).to(self.device), torch.tensor(batchY).to(self.device);
...
def setup(opt, split):
    ...
    dataset = np.load(os.path.join(opt.data, opt.dataset, 'btftpfp_wav{}.npz'.format(opt.fs // 1000)), allow_pickle=True)
